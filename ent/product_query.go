...
func (pq *ProductQuery) QueryBelongs() *CategoryQuery {
...
func (pq *ProductQuery) Clone() *ProductQuery {
	...
	return &ProductQuery{
		...
		withBelongs: pq.withBelongs.Clone(),
...
func (pq *ProductQuery) WithBelongs(opts ...func(*CategoryQuery)) *ProductQuery {
	query := &CategoryQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withBelongs = query
	return pq
}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withBelongs; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Product)
		for i := range nodes {
			if fk := nodes[i].category_contains; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_contains" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Belongs = n
			}
		}
	}
