...
func (cq *CategoryQuery) QueryContains() *ProductQuery {
...
func (cq *CategoryQuery) Clone() *CategoryQuery {
	...
	return &CategoryQuery{
		...
		withContains: cq.withContains.Clone(),
...
func (cq *CategoryQuery) WithContains(opts ...func(*ProductQuery)) *CategoryQuery {
	query := &ProductQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withContains = query
	return cq
}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withContains; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Category)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Contains = []*Product{}
		}
		query.withFKs = true
		query.Where(predicate.Product(func(s *sql.Selector) {
			s.Where(sql.InValues(category.ContainsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.category_contains
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_contains" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_contains" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Contains = append(node.Edges.Contains, n)
		}
	}
	...
	if query := cq.withParent; query != nil {
		...
		nodeids := make(map[int][]*Category)
		for i := range nodes {
			if fk := nodes[i].category_children; fk != nil {
				ids = append(ids, *fk)
